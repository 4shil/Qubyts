...
const defaultColors = ['#020810', '#041428', '#00141f'];
...
export default function LiquidEther({
    ...
    useEffect(() => {
        ...
        class MouseClass {
            ...
            _onMouseMove = (event) => this.onDocumentMouseMove(event);
            _onTouchStart = (event) => this.onDocumentTouchStart(event);
            _onTouchMove = (event) => this.onDocumentTouchMove(event);
        ...
        class ExternalForce extends ShaderPass {
            ...
            update(props = {}) {
                ...
                uniforms.center.value.set(Math.min(Math.max(Mouse.coords.x, -1 + cursorSize * cellScale.x), 1 - cursorSize * cellScale.x), Math.min(Math.max(Mouse.coords.y, -1 + cursorSize * cellScale.y), 1 - cursorSize * cellScale.y));
        ...
        class Divergence extends ShaderPass {
            ...
            update(args = {}) {
                if (args.vel) this.uniforms.velocity.value = args.vel.texture;
                super.update();
            }
        }
        ...
        class WebGLManager {
            ...
            dispose() { try { window.removeEventListener('resize', this._resize); if (this._onVisibility) document.removeEventListener('visibilitychange', this._onVisibility); Mouse.dispose(); if (Common.renderer) { const canvas = Common.renderer.domElement; if (canvas?.parentNode) canvas.parentNode.removeChild(canvas); Common.renderer.dispose(); } } catch { } }
        }
